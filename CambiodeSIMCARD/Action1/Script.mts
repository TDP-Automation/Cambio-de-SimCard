...
Sub IngresoGrupoSIM()
	...
	wait 1
	JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Negociar Configuración").JavaEdit("Buscar por nombre:").Set "Grupo de SIM"
	JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Negociar Configuración").JavaButton("Buscar").Click
	wait 2
...
Sub IngresoIMEI()
 	...
 	wait 1
	JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Negociar Configuración").JavaEdit("Buscar por nombre:").Set "Número IMEI"
	JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Negociar Configuración").JavaButton("Buscar").Click
	wait 2	
	dim Iterator , filas, j,h
	filas = JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Negociar Configuración").JavaTable("Mostrar atributos:").GetROProperty("rows")
...
''''''''Seleccion Opciones de SIM
	...
	''Pantalla de Equipo
		...
		If JavaWindow("Ejecutivo de interacción").JavaDialog("Negociar Configuración_2").JavaObject("PanelNative$JXPanel").Exist Then
			...
			wait 2
			While ((JavaWindow("Ejecutivo de interacción").JavaDialog("Negociar Configuración_2").JavaStaticText("Mostrando 1-6 de 20 equipos").Exist) OR (JavaWindow("Ejecutivo de interacción").JavaDialog("Mensaje").Exist))= false
					wait 1
			Wend
			 wait 2
			
			While JavaWindow("Ejecutivo de interacción").JavaDialog("Negociar Configuración_2").JavaCheckBox("Seleccionar_4").Exist = False
					wait 1
			Wend
				wait 5
			JavaWindow("Ejecutivo de interacción").JavaDialog("Negociar Configuración_2").JavaCheckBox("Seleccionar_4").Set "ON"
		...
		Else
			...
			Wait 2
			t = 0
			While ((JavaWindow("Ejecutivo de interacción").JavaDialog("Negociar Configuración_2").JavaStaticText("Mostrando 1-6 de 20 equipos").Exist) or (JavaWindow("Ejecutivo de interacción").JavaDialog("Negociar Configuración_2").JavaButton("Equipo").Exist) Or (JavaWindow("Ejecutivo de interacción").JavaDialog("Mensaje").Exist) Or (JavaWindow("Ejecutivo de interacción").JavaDialog("Negociar Configuración_2").JavaDialog("Mensaje").Exist)) = False
				Wait 1
				t = t + 1
				If (t >= 180) Then
					DataTable("s_Resultado", dtLocalSheet) = "Fallido"
...
End Sub
Sub pagoInmediato
		While JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Negociar Pago (Orden 599306A").JavaEdit("ID del cliente:").Exist = False
			wait 1
		Wend
		Dim textId
		...
		Dim finExterno
		finExterno =JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Negociar Pago (Orden 599306A").JavaCheckBox("Financiamiento Externo").GetROProperty("enabled")
		If JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Negociar Pago (Orden 599306A").JavaCheckBox("Financiamiento Externo").Exist = True and finExterno = "1" Then
...
'Bucle que espera el envío de la orden
	...
	Wend
		...
		If JavaWindow("Ejecutivo de interacción").JavaDialog("Mensajes de validación").JavaButton("Aceptar").Exist = True Then
			...
			varlog2= Replace(varlog, "<html>","")
			varlog = Instr(1,varlog,"Por favor considere que se agrego un cargo de penalidad a la acción de orden")
			wait 1
			'varlog2 = Por favor considere que se agrego un cargo de penalidad a la acción de orden 1062180 bajo el componente Compromiso de Equipo.
			If (varlog = "7") Then
...
Sub PagoManual()
	...
	While JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Buscar: Grupo de órdenes").JavaButton("Finalizar compra y activar").Exist=False
			wait 1
	Wend
	wait 3
...
'
'				JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Buscar: Grupo de órdenes").JavaButton("1 Registros").GetROProperty("attached text")
'
				tiempo=tiempo+1
				wait 1
			End If
